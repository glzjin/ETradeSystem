<%@ page import="Helpers.FavorHelper" %>
<%@ page import="Helpers.CartHelper" %>
    ...
    <div class="box box-default">
        ...
        <div class="box-footer clearfix">
            <form role="form" method="post">
                ...
                <c:if test="${FavorHelper.hasFavor(user, Item)}">
                    <button type="submit" class="btn btn-danger" onclick="javascript: form.action='./favor?id=${Item.getId()}';"><span class="fa fa-heart"> 已收藏</span></button>
                </c:if>
                <c:if test="${!FavorHelper.hasFavor(user, Item)}">
                    <button type="submit" class="btn btn-danger" onclick="javascript: form.action='./favor?id=${Item.getId()}';"><span class="fa fa-heart-o"> 收藏</span></button>
                </c:if>
                <c:if test="${CartHelper.hasCart(user, Item)}">
                    <button type="submit" class="btn btn-info" onclick="javascript: form.action='./cart?id=${Item.getId()}';"><span class="fa fa-cart-plus"> 已添加到购物车，数量 + 1</span></button>
                </c:if>
                <c:if test="${!CartHelper.hasCart(user, Item)}">
                    <button type="submit" class="btn btn-info" onclick="javascript: form.action='./cart?id=${Item.getId()}';"><span class="fa fa-opencart"> 添加到购物车</span></button>
                </c:if>
                <a role="button" class="btn btn-info" href="/buyer/cart_infos"><span class="fa fa-cart-plus"> 购物车结账</span></a>
            </form>
        </div>
        <!-- /.box-footer -->
    </div>
