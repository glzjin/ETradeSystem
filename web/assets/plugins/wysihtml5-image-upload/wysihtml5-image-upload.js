						...
						'<div class="modal-body">' +
						  	'<p>' +
								'<span class="btn btn-primary fileinput-button">' +
									...
									'<span>点击选择文件</span>' +
									'<input id="fileupload" type="file" name="files[]" data-url="https://etrade-image.wetolink.com/index.php" accept="image/png,image/jpg,image/bmp" multiple>' +
								'</span>' +
							'</p>' +
		...
		initInsertImage: function(toolbar) {
			...
			var insertImage = function() {
				...
				$('#files img').each(function() {
					...
					if (url) {
						self.editor.composer.commands.exec("insertHTML", "<img src=\"" + url + "\" class=\"img-responsive pad\"'/>");
					}
				});
				...
				if (!activeButton) {
					...
					var updateInsertBtn = function () {
						...
						if (imgCount) {
							...
							if (imgCount === 1) {
								insertButton.text('插入图片');
							} else {
								insertButton.text('插入 ' + imgCount + ' 张图片');
							}
						} else {
							insertButton.addClass('disabled').text('插入图片');
						}
					};
					...
					$('#fileupload').fileupload({
						...
						always: function (e, data) {
							if (data.hasOwnProperty('result')) {
								...
								$('#filealerts').append($('<div class="alert alert-danger">上传失败！</div>'));
							}
						},
